...
const IndianProductList = () => {
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              const response = await fetch('https://mongodb-products-api.onrender.com/api/routes/indianproduct');
              if (!response.ok) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              {products.map(product => (
                  <div key={product.id} className="border border-gray-300 rounded-lg p-5 text-center">
                      <h2 className="text-green-600 font-bold">{product.name}</h2>
...
export default IndianProductList;